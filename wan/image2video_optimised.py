...
"""
Optimised image-to-video pipeline with TeaCache-style step caching (Step 2).

``WanI2V_Optimised`` mirrors ``WanT2V_Optimised``: it subclasses ``WanI2V``,
binds ``cached_dit_forward`` onto both MoE experts after construction, and
overrides ``generate()`` with the minimal caching edits. With
``cache_threshold == 0.0`` (default) behaviour is identical to ``WanI2V``.
"""
import gc
import math
import random
...
class WanI2V_Optimised(WanI2V):

    def __init__(self, *args, cache_threshold=0.0, **kwargs):
        if kwargs.get('use_sp', False):
            raise NotImplementedError(
                "WanI2V_Optimised step caching is single-GPU only for now; "
    ...
    def generate(self,
                 ...
                 seed=-1,
                 offload_model=True):
        r"""Identical to ``WanI2V.generate`` plus TeaCache step caching."""
        # preprocess
        ...
        with (
            ...
            for i, t in enumerate(tqdm(timesteps)):
                ...
                # Force a real compute on the first/last 2 steps and at expert
                # swaps.
